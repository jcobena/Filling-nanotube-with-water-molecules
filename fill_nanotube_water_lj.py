...
def main():
    ...
    with open('water_pdb.txt', 'a+') as waterout, open('water_conect.txt', 'w+') as water_conect_out, \
        ...
        open('sicnt_{}_{}_{}.pdb'.format(m,n,box), 'r') as file_get_center_tube, open('final_pdb_{}.pdb'.format(rho), 'w') as final_output:
...
#getting the water. 
        ...
        oxygens = assign_positions_to_oxygens_list_pdb_new(D, L, oxygen_quantity, bond_length, cylinder_x, cylinder_y, safety_clearance, startn, sigma_O, eps_O, cutoff)
...
def assign_positions_to_oxygens_list_pdb_new(Diameter, Length, number_of_atoms, bond_length, cylinder_x, cylinder_y, safety_clearance, startn, sigma_O, eps_O, cutoff):
    ...
    while len(list_positions_0) <= number_of_atoms:
        ...
        if r_test < r:
            ...
            if len(list_positions_0) > 3:
                pe = pe_pairwise_LJ(list_positions_0, sigma_O,  eps_O,  cutoff)
                
                if pe > pe_0:
                    list_positions_0.pop()
                    print(len(list_positions_0))
